...
image_columns <- strsplit(args[2], ",")[[1]]
image_dirs <- strsplit(args[3], ",")[[1]]
df_aliases <- strsplit(args[4], ",")[[1]]
pdf_columns <- strsplit(args[5], ",")[[1]]
...
ui <- fluidPage(
  ...
  tags$head(
    tags$style(HTML("
      .resizable-panel { overflow: auto; resize: both; border: 1px solid #ddd; padding: 10px; }
      .data-table-container { height: 400px; overflow: auto; resize: vertical; }
      .zoom-container { text-align: center; margin-bottom: 10px; }
      .zoomable-image { max-width: 100%; height: auto; transition: transform 0.25s ease; }
      .resizable-media { resize: both; overflow: auto; border: 1px solid #ddd; padding: 10px; width: 100%; height: auto; }
    "))
  ),
  ...
  fluidRow(
    column(
      ...
      downloadButton("download_filtered_df", "Download Filtered Data", class = "btn-info")
    )
  ),
...
server <- function(input, output, session) {
  ...
  output$data_table <- renderDT({
    ...
    datatable(df, escape = FALSE, filter = "top", options = list(
      autoWidth = TRUE,
      dom = 'Bfrtip',
  ...
  output$download_filtered_df <- downloadHandler(
    ...
    content = function(file) {
      df <- filtered_data()
      if (!is.null(df)) {
        # Access the rows selected by the user in the DataTable
        filtered_rows <- input$data_table_rows_all
        if (!is.null(filtered_rows)) {
          write.table(df[filtered_rows, ], file, sep = "\t", row.names = FALSE, quote = FALSE)
        }
      }
    }
  )
